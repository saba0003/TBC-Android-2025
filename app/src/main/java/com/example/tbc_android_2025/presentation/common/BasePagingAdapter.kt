...
abstract class BasePagingAdapter<T : BasePagingAdapter.HasId<*>, VB : Binding>(
    private val inflater: ViewBindingInflater<VB>,
    private val onClick: ((T) -> Unit)? = null,
) : PagingDataAdapter<T, BasePagingAdapter.BaseViewHolder<VB>>(BaseDiffItemCallback()) {
        ...
        onClick?.let { listener ->
            binding.root.setOnClickListener {
                holder.bindingAdapterPosition
                    ...
                    ?.let { position ->
                        getItem(position = position)?.let { item -> listener(item) }
                    }
            }
        }
    ...
    override fun onBindViewHolder(holder: BaseViewHolder<VB>, position: Int) {
        getItem(position)?.let { item -> holder.bind { bind(binding = it, item = item) } }
    }
